...
from utils import flatten_list, cords, distance, loss, Optimizer, overall_loss
...
class Neuron:
    ...
    def optimize(self, rewards, optimizer):
        overall_reward = 0
        for (dendrite_id, axon_id), reward in rewards.items():
            optim = optimizer(reward)
            self.w_dendrites[dendrite_id] += optim
            self.w_axons[axon_id] += optim
            overall_reward += reward
        self.threshold += optimizer(overall_reward)
...
class InputNeuron(Neuron):
    ...
    def optimize(self, rewards, optimizer):
        overall_reward = 0
        for (_, axon_id), reward in rewards.items():
            optim = optimizer(reward)
            self.w_axons[axon_id] += optim
            overall_reward += reward
        self.threshold += optimizer(overall_reward)
...
class OutputNeuron(Neuron):
    ...
    def optimize(self, rewards, optimizer):
        overall_reward = 0
        for (dendrite_id, _), reward in rewards.items():
            optim = optimizer(reward)
            self.w_dendrites[dendrite_id] += optim
            overall_reward += reward
        self.threshold += optimizer(overall_reward)
...
class Net:
    def __init__(self, n_neurons=None, synapses_count=(None, None), in_size: tuple=tuple(), out_size: tuple=tuple(), optimizer='simple'):
        ...
        self.state = 'train'
        self.graph = []
        self.optimizer = Optimizer(optimizer, lr=0.001)
    ...
    def fit(self, epochs, steps, inputs, outputs, clear_signals=False, verbose=False):
        for epoch in range(epochs):
            loss_temp = 0
            for input, output in zip(inputs, outputs):
                self.add_input(input)
                self.propagate(steps)
                loss_temp += overall_loss(self.out, output.flatten())
                self.backwards(output.flatten())
                if clear_signals:
                    self.signals = []
            print(f'Epoch: {epoch} | Loss: {loss_temp}')
    ...
    def generate_neurons(self, neuron_function):
        ...
        if len(self.out_size):
            self.out = np.zeros(self.out_size).flatten()
            for j in range(self.out.size):
                id = uuid4().time
    ...
    def merge(self, net, self_2_net_merge_neurons_n, net_2_self_merge_neurons_n, new_axon, new_dendrite):
        raise NotImplementedError('Needs modification for clearing axons and dendrites of input and output neurons')
        #
        # new = deepcopy(self)
    ...
    def compute_gradients(self, target):
        gradient = {s: {} for s in set([i for j in self.graph for i in j])}
        reward = loss(self.out, target)
        for signal in self.graph:
            for i in range(len(signal)-2, 0, -1):
                gradient[signal[i]][(signal[i-1], signal[i+1])] = reward
            gradient[signal[-1]][(signal[-2], None)] = reward
            gradient[signal[0]][(None, signal[1])] = reward
        return gradient

    def backwards(self, target):
        # print(self.out.reshape(5, 5))
        gradients = self.compute_gradients(target)
        for neuron, optim_params in gradients.items():
            self.neurons[neuron].optimize(optim_params, self.optimizer)
        self.optimizer.step()
        self.graph = []
        self.out = np.zeros(self.out_size).flatten()

    def show_network_state(self):
